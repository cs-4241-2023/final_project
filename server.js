...
const checkCreateAccount=async (req)=>{
    ...
    if(info === null || info === undefined){
        let acc = {'username': req.body.Username, 'password': req.body.Password,'score':0,'rate':1};
...
app.post('/update',express.json(),async (req,res)=>{
    ...
    await userCollection.updateOne({_id:new ObjectID(id)},{ $set: { score: req.body.score,rate:req.body.rate } });
    res.redirect('app.html')
})
